...
class ClientsWindow:
    ...
    def __init__(self, window):
        ...
        for col in columns:
            ...
            self.tree.column(col, width=50 if col == "ID" else 150, anchor='center')
    ...
    def load_clients(self):
        ...
        try:
            ...
            messagebox.showerror("Ошибка", f"Не удалось загрузить клиентов: {e}", parent=self.window)
    ...
    def save_client(self):
        ...
        try:
            ...
            messagebox.showinfo("Успех", msg, parent=self.window)
            self.load_clients()
            self.filter_clients()
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось сохранить клиента: {e}", parent=self.window)
    ...
    def delete_client(self):
        ...
        selected = self.tree.selection()
        if not selected:
            messagebox.showwarning("Удаление", "Выберите клиента для удаления.", parent=self.window)
            return
        ...
        if messagebox.askyesno("Подтверждение", f"Удалить клиента '{client_name}'?", parent=self.window):
            try:
                self.db.delete_client(client_id)
                messagebox.showinfo("Успех", "Клиент удалён.", parent=self.window)
                self.load_clients()
                self.filter_clients()
            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось удалить клиента: {e}", parent=self.window)
    ...
    def export_to_csv(self):
        ...
        if not self.all_clients:
            messagebox.showinfo("Экспорт", "Нет данных для экспорта.", parent=self.window)
            return
        ...
        try:
            ...
            messagebox.showinfo("Успех", f"Данные экспортированы в {os.path.basename(file_path)}", parent=self.window)
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось экспортировать: {e}", parent=self.window)
...
class ProductsWindow:
    ...
    def load_products(self):
        ...
        try:
            ...
            messagebox.showerror("Ошибка", f"Не удалось загрузить товары: {e}", parent=self.window)
    ...
    def save_product(self):
        ...
        try:
            ...
            if not name_str:
                raise ValueError("Название товара не может быть пустое.")
            if not price_str:
                raise ValueError("Поле цены обязательно для заполнения.")
            if not stock_str:
                raise ValueError("Поле количество обязательно для заполнения.")
            ...
            messagebox.showerror("Ошибка", f"Не удалось сохранить товар: {e}", parent=self.window)
    ...
    def delete_product(self):
        ...
        selected = self.tree.selection()
        if not selected:
            messagebox.showwarning("Удаление", "Выберите товар для удаления.", parent=self.window)
            return
        ...
        if messagebox.askyesno("Подтверждение", f"Удалить товар '{product_name}'?", parent=self.window):
            try:
                self.db.delete_product(product_id)
                messagebox.showinfo("Успех", "Товар удалён.", parent=self.window)
                self.load_products()
                self.filter_products()
            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось удалить товар: {e}", parent=self.window)
...
class OrdersWindow:
    ...
    def __init__(self, window):
        ...
        columns = ("ID", "Клиент", "Товар", "Количество", "Дата заказа")
    ...
    def load_orders(self):
        ...
        try:
            ...
            messagebox.showerror("Ошибка", f"Не удалось загрузить заказы: {e}", parent=self.window)
    ...
    def save_order(self):
        ...
        try:
            ...
            messagebox.showerror("Ошибка", "Количество должно быть целым числом.", parent=self.window)
            return

        if not client_str or not product_str or quantity <= 0:
            messagebox.showerror("Ошибка", "Все поля обязательны, количество > 0.", parent=self.window)
            return
        ...
        try:
            ...
            if self.current_order_id is None:
                ...
                #     client_id = self.db.get_client_id(client_str)[0]
                # except Exception as e:
                #     messagebox.showerror("Ошибка", f"Данного клиента нет в БД, добавьте сначала нового клиента.{e}",
                #                          parent=self.window)
                #     return
                # try:
                #     product_id = self.db.get_product_id(product_str)[0]
                # except Exception as e:
                #     messagebox.showerror("Ошибка", f"Данного товара нет в БД, добавьте сначала новый товар.{e}",
                #                          parent=self.window)
                #     return

                self.db.insert_order(client_id, product_id, quantity, order_date)
                messagebox.showinfo("Успех", "Заказ добавлен!", parent=self.window)
                self.clear_fields()
            else:
                self.db.update_order(self.current_order_id, client_id, product_id, quantity, order_date)
                messagebox.showinfo("Успех", "Заказ обновлён!", parent=self.window)
                self.clear_fields()

            self.load_orders()
            self.filter_orders()
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось сохранить заказ: {e}", parent=self.window)
    ...
    def delete_order(self):
        ...
        selected = self.tree.selection()
        if not selected:
            messagebox.showwarning("Удаление", "Выберите заказ для удаления.", parent=self.window)
            return

        item = self.tree.item(selected[0])
        order_id = item['values'][0]

        if messagebox.askyesno("Подтверждение", f"Удалить заказ ID {order_id}?", parent=self.window):
            try:
                self.db.delete_order(order_id)
                messagebox.showinfo("Успех", "Заказ удалён.", parent=self.window)
                self.load_orders()
                self.filter_orders()
            except Exception as e:
                messagebox.showerror("Ошибка", f"Не удалось удалить заказ: {e}", parent=self.window)
...
class StatsWindow:
    ...
    def show_top_5_clients(self):
        ...
        if not results:
            messagebox.showinfo("Статистика", "Нет данных для отображения.", parent=self.window)
            return
    ...
    def show_clients_products_graph(self):
        ...
        if not edges:
            messagebox.showinfo("Граф", "Нет данных для построения графа.", parent=self.window)
            return
        ...
        if len(G.nodes) == 0:
            messagebox.showinfo("Граф", "Нет узлов для отображения после фильтрации.", parent=self.window)
            return
